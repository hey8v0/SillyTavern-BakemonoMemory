...
- 根因审查发现 `ensureState()` 曾在几乎每次状态读取时重复清洗全部聊天状态并重建 `memoryRecords`；现在每个聊天状态只做一次结构迁移，聊天长度减少时才重新清理越界引用，派生记忆记录只在需要显示的页面生成。
...
- `memoryRecords` 与 `scanPreview` 改为聊天状态里的非枚举派生数据，后续保存聊天元数据时不再重复写入这两份 UI 缓存。
- 版本升级到 `1.1.28`；契约测试增加关闭工作台、后台队列、大聊天扫描与派生缓存持久化边界。
...
6. 批量应用摘要是否仍会导致手机酒馆卡顿；`1.1.28` 已降低隐藏工作台渲染和扫描峰值，需要继续用几百楼真实档观察。
