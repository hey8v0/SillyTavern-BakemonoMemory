...
- 当前版本 1.7.3，分支 main，远端 hey8v0/SillyTavern-BakemonoMemory。先查 Git，不从旧对话推断。
...
- 自定义提示词不被升级覆盖；维护设置可载入默认后应用。已应用指令与表单草稿分离，预设跨聊天共享。
...
| 状态注入/预算/检索 | rp-core/context.js、memory.js；features/injection-service.js |
...
| 本轮注入预览 | features/overview-token-manifest.js、injection-preview.js；只读当前配置，不冒充已发送快照 |
...
- 全量 tests、运行模块语法、CSS、离线 DOM、显示过滤、备份、diff 检查；命令见测试说明。固定模型输出测试不证明真实语义准确率。
