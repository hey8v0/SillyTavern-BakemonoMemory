...
- 当前版本 `1.7.0-beta.1`，开发与发布分支为 `main`；测试分支已合入。实际提交、未提交修改和远端状态先查 Git，不从旧对话推断。
...
- 随正文协议只在阅读区隐藏，编辑框和聊天导出保留原始块；独立提取不追加正文。不要为了隐藏协议删掉聊天原文。
...
- **聊天／全局分离**：聊天记忆在 `chat_metadata`，共享设置与预设在 `extension_settings`。后台任务不读取隐藏表单，不提升整份运行状态为全局配置；开关只提交开关，合法的 `false / 0 / ""` 不当缺失值。
...
- 发布基线：408 项 Node 回归、130 个运行模块语法解析、完整 CSS、13 组离线界面／存储检查通过。具体覆盖、运行方式与测量见 [TESTING_v1.7.0.md](TESTING_v1.7.0.md)。
