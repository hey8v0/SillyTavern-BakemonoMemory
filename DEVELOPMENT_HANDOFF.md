...
- 当前版本 1.9.0，分支 main，远端 hey8v0/SillyTavern-BakemonoMemory。先查 Git，不从旧对话推断。
...
- 总结页路由是 preview；异常摘要跳转必须连同真实面板检查。向量计数/字数输入按整数校验，默认值必须通过原生浏览器 validity；错误字段展开定位且保留输入。tests/browser/settings-save-native.mjs 需外部 Playwright 与浏览器，命令见 CHANGELOG_UI_1.8.7.md。
...
- 自动总结只检查所选批次内的缺口，旧异常单独列出；同楼有效替代摘要可继续使用。选材、补写判定、楼层覆盖与已总结正文隐藏共用 summarySourceFloors 的当前来源位置，不按首尾范围补齐空洞。补写跳过有效已存记忆及待处理同源任务，失效或失败任务不冒充已保存；回归 tests/summary-material-isolation.test.mjs。
- 摘要来源选择统一写入旧配置字段，保留旧手动及组合方式；运行错误不进全局预设。
- 摘要方式向导只填表单，由保存栏应用；来源与工作流、记忆策略、阶段材料、输出风格按 workflowForSummarySource 一起保存（插件内摘要须配 generic 策略才会注入）。注入开关、深度、角色只在注入页。设置页的保存入口只有顶部保存栏。RP 失败去重绑定来源与协议哈希，修正输出可重读；独立调用失败仍需手动重试。
