...
维护设置 → 剧情状态提示词，可编辑、应用、另存预设。未修改的旧内置提示词自动切换为精简版；自定义文本及预设保留，可载入“默认剧情状态”后应用。

提示词版本 3，事件协议仍为 2。短引用仅用于模型输出，插件映射回原存档 ID；不改存档身份。续写现状与维护指令分开注入，不再附带完整投影或近期事件 JSON。旧协议与名称引用继续兼容。
