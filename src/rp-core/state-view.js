...
export function describeStateValues(data, projection) {
    ...
    return Object.entries(data.values || {}).map(([field, value]) => {
...
export function buildStatePage(core, view, navigation = {}) {
    ...
    if (tab === 'people') rows = [...(filter !== 'relationships' ? entityRows('people', projection.people) : []), ...(filter !== 'people' ? entityRows('relationships', projection.relationships) : [])];
        ...
        if (filter === 'pending') rows = core.candidates.filter(item => visible(item) && isCurrentRpRecord(view, item) && ['pending', 'rejected'].includes(item.status)).map(item => ({ kind: 'candidate', id: item.id,
