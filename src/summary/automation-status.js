...
export function stageAutomationStatus(state, materials, { batch = materials.targets, records = [], busy = false } = {}) {
    ...
    if (materials.invalid?.length) return status('invalid', '摘要材料需要检查', materials.invalid.slice(0, 3).join('；'),
        { tab: 'scan', label: '检查读取规则' });
    const gaps = findTargetContinuityGaps(batch, records);
