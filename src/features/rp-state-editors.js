...
export function createRpStateEditors({ escapeHtml: esc, button, help }) {
    ...
    function renderEdit(edit, projection) {
        return `<section class="rp-edit-form">${!edit.create && edit.kind !== "temporary" ? '<label class="rp-edit-field">修改方式<select class="text_pole" data-rp-edit-intent><option value="change">剧情现在发生变化</option><option value="correction"' + (edit.intent === "correction" ? " selected" : "") + '>修正过去记错的值（不改写旧历史快照）</option></select></label>' : ""}<h3 tabindex="-1">修改${({ people: '人物', relationships: '关系', plans: '约定', items: '物品', locations: '地点', clock: '剧情时间', scene: '当前场景', claims: '说法', observations: '观察', temporary: '临时状态' })[edit.kind]}</h3>${specs[edit.kind].map(([field, label, type]) => {
