...
const specs = {
    ...
    plans: [['title', '约定'], ['participants', '参与者', 'participants'], ['status', '状态', ['proposed', 'accepted', 'completed', 'cancelled', 'failed']], ['due', '约定日期'], ['dueDescription', '期限描述（日期不明确时）'], ['outcome', '结果']],
...
export function createRpStateEditors({ escapeHtml: esc, button, help }) {
    ...
    function renderEdit(edit, projection) {
        return `<section class="rp-edit-form"><h3 tabindex="-1">${edit.create ? '添加' : '修改'}${({ people: '人物', relationships: '关系', plans: '约定', items: '物品', locations: '地点', clock: '剧情时间', scene: '当前场景', claims: '说法', observations: '推测', temporary: '临时状态' })[edit.kind]}</h3>${!edit.create && edit.kind !== "temporary" ? '<label class="rp-edit-field">这次修改是<select class="text_pole" data-rp-edit-intent><option value="change">剧情里发生了变化</option><option value="correction"' + (edit.intent === "correction" ? " selected" : "") + '>之前记错了，更正（旧快照保留）</option></select></label>' : ""}${specs[edit.kind].map(([field, label, type]) => {
    ...
    function readEdit(root, edit) {
        edit.intent = root.querySelector('[data-rp-edit-intent]')?.value || edit.intent || 'change';
