...
export function createRpStatePresentation({ escapeHtml: esc }) {
    ...
    function scene(view, lastBatch, historicalFloor) {
        ...
        const places = positions.slice(0, 3).map(id => locationTrail(view, id)).filter(Boolean);
        const located = view.people.filter(item => item.location).slice(0, 4);
    ...
    return { rows, scene, entityDetail, empty, fields, chip, head };
}
