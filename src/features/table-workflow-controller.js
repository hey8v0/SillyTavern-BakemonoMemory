export function createTableWorkflowController({
    ...
    const runningStates = new WeakSet();
    async function processLatestTableEdit(options = {}) {
        ...
        const sourceSignature = JSON.stringify(blocks);
        if (!options.manual && failedSources.get(state) === sourceSignature) return;
